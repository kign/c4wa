(module
  ...
  (data (i32.const 1024) "Verifying: %.6f, %.6f, %.6f\5Cn\00(%.6f,%.6f)%c(%.6f,%.6f) = (%.6f,%.6f)\5Cn\00")
  (func $_new (result i32)
    (local $res i32)
    (set_local $res (i32.add (global.get $m) (i32.const 2048)))
    (global.set $m (i32.add (global.get $m) (i32.const 8)))
    (get_local $res))
  ...
  (func $add (param $a i32) (param $b i32) (param $c i32)
    (f32.store (get_local $c) (f32.add (f32.load (get_local $a)) (f32.load (get_local $b))))
    (f32.store (i32.add (get_local $c) (i32.const 4)) (f32.add (f32.load (i32.add (get_local $a) (i32.const 4))) (f32.load (i32.add (get_local $b) (i32.const 4))))))
