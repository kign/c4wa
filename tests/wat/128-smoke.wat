(module
  ...
  (data (i32.const 1024) "Result is %.6f\0A\00[Testing compile-time <<]\0A\002^63 = %.15e [unsigned]\0A\002^31 = %.2f [signed]\0A\002^31 = %.2f [unsigned]\0A\00[Testing run-time <<]\0A\00")
  ...
  (func $main (export "main") (result i32)
    ...
    (call $printf (i32.const 1067) (global.get $@stack))
    (set_local $fltVal (f32.const -2.147483648E9))
    (f64.store (global.get $@stack) (f64.promote_f32 (get_local $fltVal)))
    (call $printf (i32.const 1092) (global.get $@stack))
    (set_local $fltVal (f32.const 2.147483648E9))
    (f64.store (global.get $@stack) (f64.promote_f32 (get_local $fltVal)))
    (call $printf (i32.const 1114) (global.get $@stack))
    (call $printf (i32.const 1138) (global.get $@stack))
